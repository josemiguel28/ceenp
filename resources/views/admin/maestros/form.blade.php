<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-md">
        <h2 class="text-2xl font-semibold text-gray-800 mb-4">
            {{ isset($maestro) ? 'Editar Maestro' : 'Registrar Maestro' }}
        </h2>
        <form
            action="{{ isset($maestro) ? route('maestros.update', $maestro->id) : route('maestros.store') }}"
            method="POST"
            class="space-y-6"
            novalidate
        >
            ...
            <!-- Campo: Nombre -->
            <div>
                <x-input-label for="name" :value="__('Nombre del maestro')"/>
                <x-text-input
                    type="text"
                    name="name"
                    id="name"
            ...
            <!-- Campo: Correo -->
            <div>
                <x-input-label for="email" :value="__('Correo del maestro')"/>
                <x-text-input
                    type="email"
                    name="email"
                    id="email"
            ...
            <!-- Campo: Matrícula -->
            <div>
                <x-input-label for="matricula" :value="__('Ingrese la matricula del maestro')"/>
                <x-text-input
                    type="text"
                    name="matricula"
                    id="matricula"
                    class="block w-full !py-3.5 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                    placeholder="Ingresa la matrícula del maestro"
                    :value="old('matricula', $maestro->matricula ?? '')"
                    required
                />
                @error('matricula')
                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
            ...
            @if(request()->routeIs('maestros.edit'))
                <!-- Campo: Estado -->
                <div>
                    <x-input-label for="estado" :value="__('Estado del maestro')"/>
                    <select
                        name="status"
                        id="estado"
                        class="block w-full !py-3.5 border border-gray-300 mb-6 rounded-lg focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                    >
                        <option value="">Seleccione el estado del maestro</option>
                        <option value="1" {{ old('estado', $maestro->status ?? '') == 1 ? 'selected' : '' }}>Activo
                        </option>
                        <option value="0" {{ old('estado', $maestro->status ?? '') == 0 ? 'selected' : '' }}>
                            Inactivo
                        </option>
                    </select>
                </div>
            ...
            <div
                ...
                <x-input-label for="materias" :value="__('Seleccione las materias del maestro')"/>
            ...
            <div class="text-right">
                ...
                >
                    {{ isset($maestro) ? 'Guardar Cambios' : 'Crear maestro' }}
                </x-primary-button>
            </div>
        </form>
    </div>
