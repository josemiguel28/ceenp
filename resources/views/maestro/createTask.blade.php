...
@section('content')
    <div class="max-w-3xl mx-auto bg-white p-6 rounded-lg shadow-md">
        ...
        <p class="text-gray-600 mb-6">Materia: <span class="font-semibold text-blue-700">{{ $materia->nombre }}</span></p>
        ...
        <form action="{{ route('maestro.store.task', $materia) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="titulo" id="titulo" class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none" required>
            </div>

            <!-- Descripción -->
            <div class="mb-4">
                <label for="descripcion" class="block text-gray-700 font-semibold mb-1">Descripción</label>
                <textarea name="descripcion" id="descripcion" class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none" rows="3"></textarea>
            </div>

            <!-- Fecha de Entrega -->
            <div class="mb-4">
                <label for="fecha_entrega" class="block text-gray-700 font-semibold mb-1">Fecha de Entrega</label>
                <input type="datetime-local" name="fecha_entrega" id="fecha_entrega" class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-500 focus:outline-none" required>
            </div>
            ...
            <div class="flex justify-between">
                <a href="{{ route('maestro.show', $materia) }}" class="bg-gray-500 text-white px-5 py-2 rounded-lg hover:bg-gray-600 transition-all">Cancelar</a>
